...
//#include <Arduino>
//in arduino
// #include <EmbeddedTemplateLibrary.h>
#include "src/Embedded_Template_Library.h"
#include "src/etl/vector.h"
#include "src/etl/map.h"
#include "src/etl/unordered_map.h"
#include "src/etl/stack.h"
using namespace etl;
...
etl::vector<etl::vector<char, 17>, 11> maze = {
        {'#', '#', '#', '#', '#', '#', '#', '#', '#', '#', '#'},
        {'#', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', '#'}, // 1.
        {'#', ' ', '#', ' ', '#', ' ', '#', '#', '#', '#', '#'},
        {'#', ' ', '#', ' ', '#', ' ', ' ', ' ', '#', ' ', '#'}, // 3.
...
etl::vector<etl::vector<char, 17>, 11> mazeSecondLevel = {
    {'#', '#', '#', '#', '#', '#', '#', '#', '#', '#', '#'},
    {'#', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', ' ', '#'}, // 1.
    {'#', ' ', '#', ' ', '#', ' ', '#', '#', '#', '#', '#'},
    {'#', ' ', '#', ' ', '#', ' ', ' ', ' ', '#', ' ', '#'}, // 3.
...
void printMaze(const etl::vector<etl::vector<char, 17>, 11>& maze){
...
void printPath(etl::stack<coord, MaxMapSize> path){
    etl::vector<etl::vector<char, 17>, 11> newMaze = maze;
...
bool checkForWall(const etl::vector<etl::vector<char, 17>, 11>& maze, const Directions& direction, const coord& currTile){
    int x = currTile.x;
    int y = currTile.y;
    switch(direction){
        case Directions::up:
...
//------------------------------------------------------------------------------------
void dijShortPath(const coord& start, const coord& end, const etl::unordered_map<coord,Tile, MaxMapSize>& map){
    etl::unordered_map<coord, bool, MaxMapSize> visited;
    etl::unordered_map<coord, int, MaxMapSize> distance;
    etl::unordered_map<coord, coord, MaxMapSize> previousPos;
    etl::stack<coord, MaxMapSize> path;
...
int checkRamp(const etl::vector<etl::vector<char, 17>, 11>& maze, const Directions& direction, const coord& currentTileCoord) {
...
//------------------------------------------------------------------------------------
void dfs(etl::unordered_map<coord, Tile, MaxMapSize>& map){
    etl::unordered_map<coord, bool, MaxMapSize> visited;
    etl::stack<coord, MaxMapSize> unvisited;
    Tile* currTile;
...
int main(){
    etl::unordered_map<coord, Tile, MaxMapSize> map;
    dfs(map);
    return 0;
}
